...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public List readAllInfor(){
        ...
        Cursor cursor = db.query(
                ...
                projection,             // The array of columns to return (pass null to get all) // Selection  //which items you need to select
                null,              // The columns for the WHERE clause
                null,          // The values for the WHERE clause       //argument
    ...
    public List readAllInfor(String userName){
        ...
        Cursor cursor = db.query(
                ...
                selectionArgs,          // The values for the WHERE clause       //argument
                null,                   // don't group the rows
                null,                   // don't filter by row groups
                sortOrder               // The sort order
        );
        ...
        while(cursor.moveToNext()) {
            ...
            userInfo.add(user); //0Index
            userInfo.add(dob);  //1Index
            userInfo.add(password); //3Index
            userInfo.add(gender);   //4Index
        }
        cursor.close();
        return userInfo;
    }
